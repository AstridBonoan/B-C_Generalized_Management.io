  ...
  return (
    <div>
      <PageHeader
        ...
        description="Authentication identity is stored separately from the employee profile shown here."
        actions={
          can(store.state, profile, 'users', 'create') ? (
            <Button type="button" onClick={() => { setError(null); setForm({ ...emptyUser(), roleId: store.state.roles[2]?.id ?? '' }) }}>
              Add user
            </Button>
          ) : null
        }
